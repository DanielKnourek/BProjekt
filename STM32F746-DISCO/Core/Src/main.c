...
//	GPIO_PinState currentStateBTN1 = GPIO_PIN_RESET;
	...
	while (1) {
		...
		if (is_set(&Flags, FT_ACTION_RECIEVE)) {
			...
			HAL_UART_Receive_DMA(&huart6, (uint8_t*) rx_buffer,
					sizeof(rx_buffer));
		}
...
static void MX_GPIO_Init(void) {
	...
	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOI, ARDUINO_D7_Pin | ARDUINO_D8_Pin | LD1_Pin,
			GPIO_PIN_RESET);
	...
	GPIO_InitStruct.Pin = ARDUINO_D7_Pin | ARDUINO_D8_Pin | LD1_Pin
			| LCD_DISP_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
