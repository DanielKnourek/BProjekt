...
# It ensures that STLink udev rules are installed so that the vscode user has permissions.

RULE_FILE="/etc/udev/rules.d/49-stlinkv2-1.rules"
...
RULES_INSTALLED=0
if [ ! -f "$RULE_FILE" ]; then
    echo "Installing STLink udev rules to host /etc/udev/rules.d/..."
    if run_elevated bash -c "cp '$DIR'/49-stlink*.rules /etc/udev/rules.d/"; then
        ...
        echo ""
        echo "  sudo bash .devcontainer/stm/udev_rules/install_host_rules.sh"
        echo ""
        echo "Alternatively, you can skip this check by creating this file:"
        echo "  $SKIP_FILE"
        echo "  touch .devcontainer/stm/udev_rules/.skip_host_rules"
        echo ""
    ...
    echo "STLink udev rules already present on host."
fi
